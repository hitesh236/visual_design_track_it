    ...
    >
      ...
      <div style={{ padding: 'var(--spacing-md)' }}>
        ...
        {(location || city_name || state_name) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8125rem', color: 'var(--color-text-muted)', marginBottom: '10px' }}>
        ...
        <div style={{ marginTop: '12px', paddingTop: '10px', borderTop: '1px dashed var(--color-border)', display: 'flex', gap: '24px' }}>
          <div>
            <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-text-muted)' }}>Check-In</div>
            <div className="time-location-override" style={{ fontSize: '13px', fontWeight: 600 }}>{formatTime(checkIn)}</div>
          </div>
          <div>
            <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-text-muted)' }}>Check-Out</div>
            <div className="time-location-override" style={{ fontSize: '13px', fontWeight: 600 }}>{formatTime(checkOut)}</div>
          </div>
        </div>
      </div>
      {notes && <div className="standard-note-bar"><strong>Note:</strong><span dangerouslySetInnerHTML={{ __html: notes }} /></div>}
    </div>
  );
}
    ...
    <>
      <style>{`
        ...
        @media (max-width: 640px) {
          .hotel-h-card { flex-direction: column !important; }
          .hotel-h-img { width: 100% !important; height: 180px !important; }
        }
        .itinerary-shell[data-forced-mobile="true"] .hotel-h-card { flex-direction: column !important; }
        .itinerary-shell[data-forced-mobile="true"] .hotel-h-img { width: 100% !important; height: 180px !important; }
      `}</style>
      <div
        className="hotel-h-card"
        style={{
          borderRadius: 'var(--radius-card)',
          border: '1px solid var(--color-border)',
          overflow: 'hidden',
      ...
      >
        {showImages && (
          <HotelImage src={firstImage} name={name} rating={rating} className="hotel-h-img" />
        )}
...
function HotelRow(props: HotelCardProps) {
  const {
    name, rating, location, city_name, state_name, mealPlan,
    roomConfigurations, notes, checkIn, checkOut, roomType, numberOfRooms
  } = props;
  const displayLocation = [location, city_name, state_name].filter(Boolean).join(', ');

  return (
    <div style={{ borderRadius: 'var(--radius-card)', backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', width: '100%' }}>
      <div style={{ padding: '14px 16px' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', marginBottom: '6px' }}>
          <div style={{ width: '40px', height: '40px', borderRadius: '10px', backgroundColor: 'var(--color-hotel)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '2px' }}>
            ...
            {rating > 0 && (
              <div style={{ display: 'flex', gap: '2px', marginBottom: '5px' }}>
                {Array.from({ length: Math.min(Math.max(rating, 0), 5) }).map((_, i) => (
                  <svg key={i} width="12" height="12" viewBox="0 0 24 24" fill="var(--color-primary)" stroke="var(--color-primary)" strokeWidth="1"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
                ))}
              </div>
            )}
            {displayLocation && <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', color: 'var(--color-text-muted)' }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink: 0 }}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg><span>{displayLocation}</span></div>}
          </div>
        </div>
      </div>
      <div style={{ borderTop: '1px solid var(--color-border)', padding: '10px 16px', paddingLeft: '68px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <div style={{ flex: 1 }}><div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-text-muted)', marginBottom: '3px' }}>Check-In</div><div style={{ fontSize: '0.875rem', fontWeight: 700 }}>{formatTime(checkIn)}</div></div>
        <div style={{ flex: 1 }}><div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-text-muted)', marginBottom: '3px' }}>Check-Out</div><div style={{ fontSize: '0.875rem', fontWeight: 700 }}>{formatTime(checkOut)}</div></div>
      </div>
      {notes && <div className="standard-note-bar"><strong>Note:</strong><span dangerouslySetInnerHTML={{ __html: notes }} /></div>}
    </div>
  );
