...
function HotelCardFull(props: HotelCardProps) {
  ...
  return (
    <div style={{ borderRadius: 'var(--radius-card)', border: '1px solid var(--color-border)', overflow: 'clip', boxShadow: 'var(--shadow-sm)', width: '100%', backgroundColor: 'var(--color-surface)' }}>
      ...
      <div style={{ padding: 'var(--spacing-md)' }}>
        ...
        {(location || city_name || state_name) && <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8125rem', color: 'var(--color-text-muted)', marginBottom: '10px' }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>{[location, city_name, state_name].filter(Boolean).join(', ')}</div>}
...
function HotelCardHorizontal(props: HotelCardProps) {
  ...
  return (
    ...
    >
      {showImages && (
        <HotelImage src={firstImage} name={name} rating={rating} className={isMobile ? "w-full h-[180px]" : "w-[220px] h-auto"} />
      )}
      <div style={{ padding: 'var(--spacing-md)', flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        {notes && <div style={{ marginTop: 'auto', paddingTop: '8px', borderTop: '1px dashed var(--color-border)', fontSize: '0.7rem', color: 'var(--color-text-muted)', fontStyle: 'italic' }}>{notes.replace(/<[^>]+>/g, '').substring(0, 100)}...</div>}
      </div>
    </div>
  );
}
