...
function ActivityCardFull({ name, description, images, showImages, showDescriptions }: ActivityCardProps) {
  ...
  return (
    <div style={{ borderRadius: 'var(--radius-card)', border: '1px solid var(--color-border)', overflow: 'clip', backgroundColor: 'var(--color-surface)', boxShadow: 'var(--shadow-card)', transition: 'box-shadow 0.2s ease' }}>
...
function ActivityCardHorizontal(props: ActivityCardProps) {
  ...
  return (
    ...
    >
      {showImages && (
        <ActivityImage src={firstImage} name={name} className={isMobile ? "w-full h-[160px]" : "w-[180px] h-auto"} />
      )}
      <div style={{ padding: 'var(--spacing-md)', flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '8px' }}>
          <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', fontWeight: 700, color: 'var(--color-text)', lineHeight: 1.3, margin: 0 }}>{name}</h3>
